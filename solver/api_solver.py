...
class TurnstileAPIServer:
    ...
    def _is_browser_reusable(self, browser, index: int) -> bool:
        """
        Decide whether a browser instance can be returned to the pool.
        Browsers without is_connected() are treated as reusable.
        """
    ...
    async def _solve_turnstile(self, task_id: str, url: str, sitekey: str, action: Optional[str] = None, cdata: Optional[str] = None):
        ...
        browser_reusable = self._is_browser_reusable(browser, index)

        try:
            if not browser_reusable:
                if self.debug:
        ...
        """)
            ...
            if not result_saved:
                ...
                try:
                    ...
                    logger.error(f"Failed to save failed task result for {task_id}: {save_e}")
            if self.debug:
